...
export function GapAnalysisPage() {
  ...
  const handleSelectFacility = async (facilityId: string) => {
    ...
    setError(null);
    try {
      const data = await gapsAPI.getByFacility(Number(facilityId));
  ...
  const handleTakeSnapshot = async () => {
    ...
    try {
      await gapsAPI.snapshot(Number(selectedFacility));
      // Reload facility detail to show new snapshot
      if (selectedFacility) {
        const data = await gapsAPI.getByFacility(Number(selectedFacility));
        setFacilityDetail(data.data);
      }
    } catch (err) {
