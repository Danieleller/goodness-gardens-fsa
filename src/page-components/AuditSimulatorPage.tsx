...
export function AuditSimulatorPage() {
    ...
    const loadFacilityData = async () => {
      ...
      try {
        const [modulesData, simulationsData] = await Promise.all([
          auditAPI.modules(Number(selectedFacility)),
          auditAPI.simulations.getAll(Number(selectedFacility)),
        ]);
  ...
  const handleStartSimulation = async () => {
    ...
    setLoading(true);
    try {
      const data = await auditAPI.simulations.create(Number(selectedFacility));
  ...
  const handleSaveResponses = async () => {
    ...
    try {
      ...
      try {
        const findingsData = await auditFindingsAPI.getAll({ simulation_id: Number(currentSimulation.id) });
