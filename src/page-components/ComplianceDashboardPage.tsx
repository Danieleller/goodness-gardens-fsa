...
export function ComplianceDashboardPage() {
    ...
    const loadComplianceData = async () => {
      ...
      try {
        const [scoreRes, matrixRes] = await Promise.all([
          complianceAPI.getScore(Number(selectedFacilityId)),
          complianceAPI.getMatrix(Number(selectedFacilityId)),
        ]);
  ...
  const handleModuleClick = async (moduleCode: string) => {
    ...
    if (!requirementsMap[moduleCode] && selectedFacilityId) {
      try {
        const res = await complianceAPI.getRequirements(moduleCode, Number(selectedFacilityId));
  ...
  const handleRunAssessment = async () => {
    ...
    try {
      const res = await complianceAPI.getScore(Number(selectedFacilityId), { save_assessment: true });
